...
- 用户偏好存储在`~/.openclaw/MEMORY.md`中
- 该文件有大小限制，旧内容会被压缩
- 更可能的是，你的偏好未被正确标记为"事实"，而是被当作了"对话历史"
- 解决方案是在`AGENTS.md`中明确写入编码规范
<!-- 修改原因：问题2 - 明确配置文件路径为 ~/.openclaw/ 目录下 -->
...
- **NanoClaw路线**：用几百行代码实现一个极简但专属于你的Agent
- **Fork路线**：基于OpenClaw源码修改，适配你的企业环境
- **IronClaw路线**：添加安全加固、审计日志、权限控制，满足合规要求（第三方开源项目）
...
```
<!-- 修改原因：问题4 - 修正架构图描述，说明Gateway到Agent Runtime通过RPC和事件系统通信，工具调用由Agent Runtime执行后再返回给Gateway -->

我们会详细解释：

- Gateway如何处理来自不同渠道（Telegram/Discord/Web）的消息
- Command Queue如何管理命令队列、保证顺序、处理并发（按渠道配置队列模式：steer/followup/collect/steer-backlog/steer+backlog/queue/interrupt）
- Agent Runtime如何分解任务、选择工具、管理状态
<!-- 修改原因：问题5 - 补充完整的队列模式列表，并说明是按渠道配置的 -->
