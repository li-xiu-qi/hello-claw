...
│                      Clients / Nodes                         │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐  │
│  │  macOS App  │  │    CLI      │  │   Web Admin UI      │  │
│  │  (Control)  │  │  (Control)  │  │    (Control)        │  │
│  └─────────────┘  └─────────────┘  └─────────────────────┘  │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐  │
│  │  macOS Node │  │   iOS Node  │  │   Android/Headless  │  │
│  │  (canvas.*) │  │ (camera.*)  │  │   (screen.record)   │  │
│  └─────────────┘  └─────────────┘  └─────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
OpenClaw 直接使用各平台的官方 SDK 接入，而非自行实现协议：
...
Gateway 负责统一身份标识。不同平台的用户 ID 格式各异，但都遵循统一规范：

- Telegram：`agent:main:telegram:dm:123` 或 `telegram:acc-1:dm:user-1`
- Discord：`agent:main:discord:channel:ops` 或 `discord:acc-1:dm:user-1`
- Slack：`agent:main:slack:cron:job:run:uuid` 或 `slack:acc-1:dm:user-1`

> 格式说明：`agent:<agentId>:<channel>:<peer>` 或 `<channel>:<account>:<type>:<id>`
...
**泳道间的并行处理**：不同用户互不等待。用户 A 的 5 分钟代码分析不会阻塞用户 B 的天气查询。
...
- **任务栈**：当前正在执行的任务及其子任务，复杂任务会被分解成多个步骤
...
| 问题 | 处理方式 | 示例 |
|------|----------|------|
| **格式错误** | 启发式自动修复 | `{ "tool": "bash" ` → 补全 `}` |
| **内容违规** | 构造新提示词要求重写 | "请严格按照 JSON 格式返回" |
| **网络超时** | 指数退避重试 | 自动重试 |
