...
模型看到可用技能列表中有`code-review.md`，描述匹配当前任务 → 模型调用`read`读取该技能 → Agent按照代码审查的流程和质量标准来回应
...
OpenClaw通过单文件大小限制和总大小限制来控制提示词大小。以下是一个概念性的预算分配建议：

| 池子 | 建议占比 | 用途 |
|------|---------|------|
| 基础系统 | 较小比例 | 安全约束、工具说明 |
| 人格定义 | 较大比例 | SOUL.md、USER.md |
| 行为规范 | 中等比例 | AGENTS.md、HEARTBEAT.md |
| 环境配置 | 中等比例 | TOOLS.md、MEMORY.md摘要 |
| 技能模块 | 中等比例 | 动态加载的skills/*.md |
| 环境上下文 | 较小比例 | 时间、目录、系统信息 |

这种分配建议基于实践经验：
- **人格定义**建议占最大份额，因为Agent的"一致性"是最宝贵的用户体验
- **技能模块**虽然重要，但动态加载机制让它不需要常驻
- **环境上下文**虽然信息量少，但必须保留足够的空间给最新状态

> **注意**：上述比例是概念性建议，而非系统强制执行的严格分配。实际限制由相关配置参数控制。
