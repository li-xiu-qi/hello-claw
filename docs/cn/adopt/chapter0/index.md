...
**AutoClaw 一键安装（推荐新手）**：智谱推出的 [AutoClaw](https://autoglm.zhipuai.cn/autoclaw)（澳龙）是国内首个一键安装版 OpenClaw 桌面客户端。下载 → 双击 → 手机号注册 → 开始用，预装 50+ 热门技能，内置龙虾专属模型 Pony-Alpha-2 和浏览器操作能力，提供免费体验额度。**如果你只想最快用上龙虾，这是最佳选择。** 支持 macOS 和 Windows，详见[第一章](/cn/adopt/chapter1/)。
...
- **AutoClaw（推荐）**：[下载地址](https://autoglm.zhipuai.cn/autoclaw)，一键安装，预装技能和模型，免费体验
- **ArkClaw**：火山引擎的 [ArkClaw](https://www.volcengine.com/product/arkclaw) 提供云端 OpenClaw，注册后即可使用

> 本教程以本地安装为主线，因为这能让你完全掌控 OpenClaw 的配置和数据。AutoClaw 和云端托管的操作方式类似，学会后可以无缝迁移。
